...
// import { Archivo } from "next/font/google";
// import { type Column, type Job } from "@/app/jobs/jobStore";
// import { createJob, getAllColumns, getAllJobs } from "@/app/jobs/actions";
// import CreateJobModal from "@/components/modals/CreateJobModal";
// import CreateListModal from "@/components/modals/CreateListModal"; // New modal import
...
//   // State for Create New List modal
...
//     if (!listName) return; // Basic validation
...
//       <KanbanProvider columns={columns} data={jobs} onDataChange={setJobs}>
//         {(column) => (
//           <div key={column.id}> {/* Wrap each column in a single element */}
//             <KanbanBoard
//               id={column.id}
//               className="bg-white p-2 shadow-lg"
//             >
...
//                     onClick={handleOpenListModal} // <-- open the CreateListModal instead of CreateJobModal
//                   >
//                     <Plus strokeWidth={5} className="h-3 w-3 text-white" />
//                   </div>
//                 </div>
...
//       <div
//         className="flex items-center justify-center bg-gray-100 h-32 cursor-pointer rounded-lg mt-4"
//         onClick={handleOpenListModal}
//       >
//         <Plus className="h-5 w-5 text-gray-500" />
//         <span className="ml-2 text-gray-500 font-medium">Create New List</span>
//       </div>
...
//       const columnsData = await getAllColumns();
//       const jobsData = await getAllJobs();
//       const filteredColumns = columnsData.filter((c) => c.name !== "CREATE_NEW");
...
//   const allColumns = [...columns, { id: "create-new-list", name: "Create New List" }];
...
//             <KanbanBoard id={column.id} key={column.id} className="bg-gray-100 p-2 shadow-lg">
//               <KanbanHeader className="border-0">
//                 <div
...
//                 >
//                   <Plus className="h-5 w-5 text-gray-500" />
//                   <span className="ml-2 text-gray-500 font-medium">{column.name}</span>
//                 </div>
//               </KanbanHeader>
//             </KanbanBoard>
//           ) : (
//             <KanbanBoard id={column.id} key={column.id} className="bg-white p-2 shadow-lg">
//               <KanbanHeader className="border-0">
//                 <div className="flex justify-between items-center gap-2">
//                   <span className={`text-md ${archivo.variable} font-semibold`}>{column.name}</span>
//                   <div
//                     className="bg-[#636AE8] hover:bg-[#5A5FD3] cursor-pointer hover:text-accent-foreground rounded-full p-2"
...
//               <KanbanCards id={column.id}>
//                 {(job) => (
//                   <KanbanCard column={column.id} id={job.id} key={job.id} name={job.name} className="bg-[#F3F4F6] border-0">
//                     <div className="flex items-start justify-between gap-2 relative">
//                       <div className="flex gap-3">
//                         {job && (
//                           <Avatar className="h-10 w-10 shrink-0">
//                             <AvatarImage src={job.image as string} className="!rounded-md" />
//                             <AvatarFallback>{job.name?.slice(0, 2)}</AvatarFallback>
//                           </Avatar>
//                         )}
//                         <div className="flex flex-col gap-1 max-w-30">
//                           <p className="mt-1 flex-1 font-medium text-xs overflow-hidden text-ellipsis whitespace-nowrap">{job.name}</p>
//                           <p className="m-0 text-muted-foreground text-2xs">{job.company as string}</p>
//                         </div>
//                       </div>
...
//                         <div
//                           className="cursor-pointer hover:bg-accent hover:text-accent-foreground rounded-full p-2"
//                           onClick={(e) => {
//                             e.stopPropagation();   // Prevent drag
//                             setOpenMenuJobId(openMenuJobId === job.id ? null : job.id);
//                           }}
//                           onPointerDown={(e) => e.stopPropagation()}  // NEW: prevent drag from starting
//                         >
//                           <Ellipsis className="h-4 w-4" />
//                         </div>
...
//                               onClick={(e) => {
//                                 e.stopPropagation();
//                                 window.open(job.url as string, "_blank");
//                                 setOpenMenuJobId(null);
//                               }}
//                             >
//                               Go to link
...
//         targetColumn={targetColumn}
//       />
//       <CreateListModal isOpen={isListModalOpen} onClose={handleCloseListModal} onSubmit={handleCreateList} />
            ...
            <KanbanBoard id={column.id} key={column.id} className="bg-white p-2 shadow-lg">
              ...
              <KanbanCards id={column.id}>
                {(job) => {
                  ...
                  return (
                  <KanbanCard column={column.id} id={jobData.id} key={jobData.id} name={jobData.name} className="bg-[#F3F4F6] border-0">
                    <div className="flex items-start justify-between gap-2 relative">
                      <div className="flex gap-3">
                        {jobData && (
                          <Avatar className="h-10 w-10 shrink-0">
                            <AvatarImage src={jobData.image as string} className="!rounded-md" />
                            <AvatarFallback>{jobData.name?.slice(0, 2)}</AvatarFallback>
                          </Avatar>
                        )}
                        <div className="flex flex-col gap-1 max-w-30">
                          <p className="mt-1 flex-1 font-medium text-xs overflow-hidden text-ellipsis whitespace-nowrap">{jobData.name}</p>
                          <p className="m-0 text-muted-foreground text-2xs">{jobData.company as string}</p>
                        </div>
                      </div>

                      {/* Dropdown menu trigger */}
                      <div className="relative" ref={openMenuJobId === jobData.id ? menuRef : undefined}>
                        <div
                          className="cursor-pointer hover:bg-accent hover:text-accent-foreground rounded-full p-2 z-10"
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            console.log("Clicked three dots for job:", jobData.id, "Current open menu:", openMenuJobId); // Debug log
                            setOpenMenuJobId(openMenuJobId === jobData.id ? null : jobData.id);
                          }}
                          onPointerDown={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                          ...
                          >
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                console.log("Go to link clicked for job:", jobData.id, jobData.name);
                                console.log("Job data:", jobData); // Log full job data
                                const url = (jobData.link as string) || (jobData.url as string) || (jobData.jobLink as string);
                                console.log("Found URL:", url); // Log the URL
                                if (url && url.trim()) {
                                  console.log("Opening URL:", url);
                                  window.open(url, "_blank");
                                } else {
                                  console.log("No URL available for this job - checking all possible fields");
                                  console.log("jobData.link:", jobData.link);
                                  console.log("jobData.url:", jobData.url);
                                  alert("No URL available for this job application");
                                }
                                setOpenMenuJobId(null);
                              }}
                            >
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                console.log("Delete clicked for job:", jobData.id, jobData.name);
                                console.log("About to call handleOpenDeleteModal with:", jobData);
